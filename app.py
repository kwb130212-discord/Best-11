...
async def on_message_delete(message: discord.Message):
    ...
    if row and row["log_channel_id"]:
        ...
        if log_ch:
            embed = discord.Embed(title="🗑️ 메시지 삭제 감지", color=discord.Color.red(), timestamp=datetime.now(KST))
            ...
            if message.attachments:
                embed.add_field(name="첨부파일", value=", ".join([att.filename for att in message.attachments]), inline=False)
            try:
...
class ScrimVoteView(discord.ui.View):
    ...
    @discord.ui.button(label="🟢 참석", style=discord.ButtonStyle.green, custom_id="scrim_attend")
    ...
    @discord.ui.button(label="🔴 불참", style=discord.ButtonStyle.danger, custom_id="scrim_absent")
    ...
    @discord.ui.button(label="🟡 미정", style=discord.ButtonStyle.secondary, custom_id="scrim_pending")
    ...
    async def update_status(self, interaction: discord.Interaction, status: str, status_kr: str):
        ...
        embed = discord.Embed(title=f"📅 스크림 일정: {scrim_info['title']}", description=f"**⏰ 일시:** {scrim_info['scrim_time']}\n**📝 내용:** {scrim_info['description'] or '설명 없음'}", color=discord.Color.blue(), timestamp=datetime.now(KST))
...
async def create_scrim(interaction: discord.Interaction, 제목: str, 일시: str, 설명: str = None):
    conn = get_conn()
    cur = conn.cursor()
    cur.execute("INSERT INTO scrim_schedules (guild_id, title, scrim_time, description, author_id, created_at) VALUES (?, ?, ?, ?, ?, ?)", (interaction.guild_id, 제목, 일시, 설명, interaction.user.id, now_kst_str()))
    scrim_id = cur.lastrowid
    conn.commit()
    conn.close()
    embed = discord.Embed(title=f"📅 스크림 일정: {제목}", description=f"**⏰ 일시:** {일시}\n**📝 내용:** {설명 or '설명 없음'}", color=discord.Color.blue(), timestamp=datetime.now(KST))
...
async def on_member_join(member: discord.Member):
    conn = get_conn()
    conn.execute("INSERT INTO user_join_counts (guild_id, user_id, join_count) VALUES (?, ?, 1) ON CONFLICT(guild_id, user_id) DO UPDATE SET join_count = join_count + 1", (member.guild.id, member.id))
    ...
    if row and row["welcome_channel_id"]:
        ...
        if ch:
            ...
            embed = discord.Embed(title="✨ 새로운 멤버 입장", description=f"환영합니다, {member.mention} 님! 🎉\n서버와 함께 즐거운 시간 보내세요!", color=discord.Color.from_rgb(85, 239, 196), timestamp=datetime.now(KST))
...
async def on_member_remove(member: discord.Member):
    ...
    if row and row["welcome_channel_id"]:
        ...
        if ch:
            now_time = datetime.now(KST).strftime("%Y년 %m월 %d일 %p %I시 %M분").replace("AM", "오전").replace("PM", "오후")
            embed = discord.Embed(title="👋 멤버 퇴장 (관리 패널)", description=f"**{member}** 님이 서버를 떠나셨습니다.\n관리자만 아래 버튼으로 즉시 제재할 수 있습니다.", color=discord.Color.from_rgb(255, 118, 117), timestamp=datetime.now(KST))
            embed.set_thumbnail(url=member.display_avatar.url)
...
class VerifyModal(discord.ui.Modal):
    def __init__(self, target_number: int):
        ...
        self.user_answer = discord.ui.TextInput(label=f"아래 인증 숫자를 입력해 주세요: [{target_number}]", placeholder=str(target_number), required=True, min_length=4, max_length=4)
        ...
        if typed_val == str(self.target_number):
            ...
            if role:
                try:
                    ...
                    await interaction.response.send_message(f"✅ **인증 완료!** `{VERIFY_ROLE_NAME}` 역할을 받으셨습니다.", ephemeral=True)
            ...
            await interaction.response.send_message("❌ **인증 실패!** 입력하신 숫자가 일치하지 않습니다.", ephemeral=True)
        ...
        if custom_id.startswith("mod_kick_") or custom_id.startswith("mod_ban_"):
            ...
            if custom_id.startswith("mod_kick_"):
                try:
                    ...
                    await interaction.response.send_message("🔨 성공적으로 해당 유저를 **추방(Kick)** 조치했습니다.", ephemeral=True)
                except Exception as e:
                ...
                try:
                    ...
                    await interaction.response.send_message("🚫 성공적으로 해당 유저를 **차단(Ban)** 조치했습니다.", ephemeral=True)
                except Exception as e:
...
async def set_welcome_channel(interaction: discord.Interaction, 채널: discord.TextChannel):
    conn = get_conn()
    conn.execute("INSERT INTO guild_settings (guild_id, welcome_channel_id) VALUES (?, ?) ON CONFLICT(guild_id) DO UPDATE SET welcome_channel_id = ?", (interaction.guild_id, 채널.id, 채널.id))
    conn.commit()
    conn.close()
...
async def unset_welcome_channel(interaction: discord.Interaction):
    conn = get_conn()
    conn.execute("UPDATE guild_settings SET welcome_channel_id = NULL WHERE guild_id = ?", (interaction.guild_id,))
    conn.commit()
    conn.close()
...
class TicketModal(discord.ui.Modal, title="🎫 티켓 지원서 작성"):
    max_tier = discord.ui.TextInput(label="최대 티어", placeholder="예: 초월자 3, 다이아몬드 1 등", required=True, max_length=50)
    level = discord.ui.TextInput(label="레벨", placeholder="예: 250레벨", required=True, max_length=30)
    nickname = discord.ui.TextInput(label="닉네임 (#태그 포함)", placeholder="예: 홍길동#KR1", required=True, max_length=50)
    mindset = discord.ui.TextInput(label="마음가짐", placeholder="간단한 각오나 가입 동기를 적어주세요.", style=discord.TextStyle.paragraph, required=True, max_length=300)
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        conn.commit()
        conn.close()
        embed = discord.Embed(title=f"🎫 {user.display_name}님의 지원 티켓", description="작성해주신 지원서 내용입니다. 관리자의 확인을 기다려주세요!", color=discord.Color.blue(), timestamp=datetime.now(KST))
...
async def ticket_panel(interaction: discord.Interaction):
    ...
    embed = discord.Embed(title=f"🎫 {server_name} 지원 및 문의 티켓", description="아래 **[🎫 티켓 열기]** 버튼을 누르면 설문창이 뜨며, 작성 완료 시 전용 채널이 생성됩니다.", color=discord.Color.green())
...
async def verify_panel(interaction: discord.Interaction):
    server_name = interaction.guild.name if interaction.guild else "서버"
    embed = discord.Embed(title=f"🔒 {server_name} 회원 인증", description="아래 **[인증하기 🔓]** 버튼을 누른 후, 안내되는 4자리 숫자를 입력해 주세요.", color=discord.Color.green())
...
async def create_notification_panel(interaction: discord.Interaction):
    await interaction.response.defer(ephemeral=True)
    embed = discord.Embed(title="🔔 알림 역할 설정", description="받으실 알림을 눌러주세요\n____________________", color=discord.Color.gold())
...
async def add_notification_button(interaction: discord.Interaction, 버튼이름: str, 지급역할: discord.Role):
    ...
    async for message in interaction.channel.history(limit=20):
        if message.author == bot.user and message.embeds and "알림 역할 설정" in message.embeds[0].title:
            target_msg = message
            break
    if not target_msg:
