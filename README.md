...
저장소의 `Procfile`도 `python best_bot.py`를 worker 시작 명령으로 사용합니다.
...
- `/확률업 역할 배율` 역할별 무료 이벤트 당첨 가중치 설정
- `/이벤트추첨 이벤트ID` 가중치 기반 무료 이벤트 추첨
- `/루에드알림 켜기:true/false` 개인별 루에드 유튜브 알림 구독
- `/루에드채널설정 채널` 루에드 유튜브 채널 및 알림 채널 설정
- 서버 보안/레이드 방어 및 입퇴장 로그

### v2 업그레이드
- `/명령어` 현재 등록된 슬래시 명령어 목록 확인
- `/봇상태` 봇 핑, 서버 수, 명령어 수, discord.py 버전 확인
- `/서버정보` 서버 기본 정보 및 아이콘 표시
- `/채팅순위 개수` 메시지 수 기반 누적 채팅 순위
- `/경고 대상 사유` 관리자 경고 기록 추가
- `/경고조회 대상` 누적 경고 및 최근 변경 시간 확인
- `/경고초기화 대상 사유` 관리자 경고 기록 초기화
- `/킥 대상 사유` 관리자 멤버 추방
- 경고/추방 기록을 SQLite에 별도로 저장
- 관리자 명령 실행 시 역할 계층과 Discord 권한을 실행 단계에서 확인

`관리자`라는 **정확한 역할 이름**을 가진 사용자는 관리자 전용 명령을 사용할 수 있습니다.

YouTube 알림은 공개 YouTube 채널 피드를 주기적으로 확인하며, 같은 서버에서 해당 채널 알림을 켠 사용자만 멘션 대상으로 처리합니다.

사진 기반 자동 지급 기능은 별도 구현 대상으로 남겨 둡니다.
